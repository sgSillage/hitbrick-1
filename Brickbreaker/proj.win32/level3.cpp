...
void level3::setPhysicsWorld(PhysicsWorld* world) { m_world = world; }
...
bool level3::init() {
	...
	//储存数据
	if (database->getBoolForKey("bool3", false)) {
		First = database->getIntegerForKey("FIRST_PLACE3");
		Second = database->getIntegerForKey("SECOND_PLACE3");
		Third = database->getIntegerForKey("THIRD_PLACE3");
		Fourth = database->getIntegerForKey("FOURTH_PLACE3");
		Fifth = database->getIntegerForKey("FIFTH_PLACE3");
	}
	else {
		database->setBoolForKey("bool3", true);
	...
	auto levelLabel = Label::create("Level: 3", "fonts/arial.ttf", 32);
	...
	auto exitbtnF = MenuItemLabel::create(labelF, CC_CALLBACK_1(level3::returnMenu, this));
	...
	vector<Sprite*>::iterator  it = boxes.begin();	
	

	schedule(schedule_selector(level3::update), 0.01f, kRepeatForever, 0.1f);
	//schedule(schedule_selector(level3::updateReadylong), 1.0f, kRepeatForever, 0.1f);
...
void level3::addSprite() {
	...
	auto bar1 = Sprite::create("blackbar.png");

	bar1->setScale(0.8f, 0.15f);
	bar1->setPosition(Vec2(visibleSize.width*0.25, visibleSize.height*0.55));
	auto blbarbody1 = PhysicsBody::createBox(blackbar->getContentSize(), PhysicsMaterial(1000.0f, 1.0f, 10.0f));

	blbarbody1->setCategoryBitmask(0xFFFFFFFF);

	blbarbody1->setCollisionBitmask(0xFFFFFFFF);

	blbarbody1->setContactTestBitmask(0xFFFFFFFF);

	blbarbody1->setGravityEnable(false);

	blbarbody1->setTag(3);

	blbarbody1->setDynamic(false);
	bar1->setPhysicsBody(blbarbody1);
	this->addChild(bar1);

	auto bar2 = Sprite::create("blackbar.png");

	bar2->setScale(0.8f, 0.15f);
	bar2->setPosition(Vec2(visibleSize.width*0.75, visibleSize.height*0.35));
	auto blbarbody2 = PhysicsBody::createBox(blackbar->getContentSize(), PhysicsMaterial(1000.0f, 1.0f, 10.0f));

	blbarbody2->setCategoryBitmask(0xFFFFFFFF);

	blbarbody2->setCollisionBitmask(0xFFFFFFFF);

	blbarbody2->setContactTestBitmask(0xFFFFFFFF);

	blbarbody2->setGravityEnable(false);

	blbarbody2->setTag(3);

	blbarbody2->setDynamic(false);
	bar2->setPhysicsBody(blbarbody2);
	this->addChild(bar2);
...
void level3::addListener() {

	auto keyboardListener = EventListenerKeyboard::create();

	keyboardListener->onKeyPressed = CC_CALLBACK_2(level3::onKeyPressed, this);

	keyboardListener->onKeyReleased = CC_CALLBACK_2(level3::onKeyReleased, this);
	...
	contactListener->onContactBegin = CC_CALLBACK_1(level3::onConcactBegin, this);
...
void level3::update(float dt) {
	...
	//ball->getPhysicsBody()->setVelocity(velocity*(1 + _scorevalue * 0.5));
}



void level3::updateReadylong(float dt)
{
	//随分数增加而加速
	Vec2 velocity = ball->getPhysicsBody()->getVelocity();
	ball->getPhysicsBody()->setVelocity(velocity*(1 + _scorevalue * 0.5));
	...
	for (int i = 1; i < 4; i++) {
		...
		while (cw <= visibleSize.width) {
			...
			if (cw == 178 && i == 2) {
				...
				auto box = Sprite::create("TNT.png");
...
void level3::onKeyPressed(EventKeyboard::KeyCode code, Event* event) {

	switch (code) {
	case cocos2d::EventKeyboard::KeyCode::KEY_LEFT_ARROW:
		if (player->getPosition().x > 100) {
			player->getPhysicsBody()->setVelocity(Vec2(-650, 0));
		}
		else {
			//player->setPosition(100, player->getPosition().y);
			player->getPhysicsBody()->setVelocity(Vec2(0, 0));
...
void level3::onKeyReleased(EventKeyboard::KeyCode code, Event* event) {
	switch (code) {
	case cocos2d::EventKeyboard::KeyCode::KEY_LEFT_ARROW:
	case cocos2d::EventKeyboard::KeyCode::KEY_RIGHT_ARROW:
		// 停止运动
	...
	case cocos2d::EventKeyboard::KeyCode::KEY_SPACE:   // 蓄力结束，小球发射
		if (spHolded == 1) {
			...
			ball->getPhysicsBody()->setVelocity(Vec2(0, spFactor * 28));
		}
		break;
	default:
		break;
...
// 碰撞检测
// Todo
bool level3::onConcactBegin(PhysicsContact & contact) {
	...
	else if (c1->getTag() == Tag::TNT && c2->getTag() == Tag::BALL) {
		//下雨
		rain->setPosition(s1->getPosition());
		addChild(rain);
		//星光
	...
	else if (c2->getTag() == Tag::TNT && c1->getTag() == Tag::BALL) {
		rain->setPosition(s2->getPosition());
		addChild(rain);
...
void level3::GameOver() {

	//排行榜
	if (_scorevalue > Fifth&&_scorevalue <= Fourth) {
		database->setIntegerForKey("FIFTH_PLACE3", _scorevalue);
		database->flush();
	}
	else if (_scorevalue > Fourth&&_scorevalue <= Third) {
		database->setIntegerForKey("FIFTH_PLACE3", Fourth);
		database->setIntegerForKey("FOURTH_PLACE3", _scorevalue);
		database->flush();
	}
	else if (_scorevalue > Third&&_scorevalue <= Second) {
		database->setIntegerForKey("FIFTH_PLACE3", Fourth);
		database->setIntegerForKey("FOURTH_PLACE3", Third);
		database->setIntegerForKey("THIRD_PLACE3", _scorevalue);
		database->flush();
	}
	else if (_scorevalue > Second&&_scorevalue <= First) {
		database->setIntegerForKey("FIFTH_PLACE3", Fourth);
		database->setIntegerForKey("FOURTH_PLACE3", Third);
		database->setIntegerForKey("THIRD_PLACE3", Second);
		database->setIntegerForKey("SECOND_PLACE3", _scorevalue);
		database->flush();
	}
		...
		database->setIntegerForKey("FOURTH_PLACE3", Third);
		database->setIntegerForKey("THIRD_PLACE3", Second);
		database->setIntegerForKey("SECOND_PLACE3", First);
		database->setIntegerForKey("FIRST_PLACE3", _scorevalue);
		database->flush();
	}
	...
	if (_scorevalue < target_score) {
		...
		auto replayBtn = MenuItemLabel::create(label2, CC_CALLBACK_1(level3::replayCallback, this));
		...
		auto exitBtn = MenuItemLabel::create(label3, CC_CALLBACK_1(level3::exitCallback, this));
	...
	auto exitbtnS = MenuItemLabel::create(labelS, CC_CALLBACK_1(level3::returnMenu, this));
...
void level3::Bluefunc() {
	onBig = true;
}


void level3::TNTfunc() {
	boxes[48]->removeFromParentAndCleanup(true);
	boxes[79]->removeFromParentAndCleanup(true);
	boxes[81]->removeFromParentAndCleanup(true);
	_scoreLabel->setString(StringUtils::format("积分：%i", _scorevalue += 150));
}