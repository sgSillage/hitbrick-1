...
{
	if (spHolded == 1) {
		readylong = spFactor / 30;
		ReadyB->setScale(readylong*0.9, 0.9);
	}
}*/
...
void level3::GameOver() {
	...
	if (_scorevalue > Fifth&&_scorevalue <= Fourth) {
		database->setIntegerForKey("FIFTH_PLACE", _scorevalue);
		database->flush();
	}
	else if (_scorevalue > Fourth&&_scorevalue <= Third) {
		database->setIntegerForKey("FIFTH_PLACE", Fourth);
		database->setIntegerForKey("FOURTH_PLACE", _scorevalue);
		database->flush();
	}
	else if (_scorevalue > Third&&_scorevalue <= Second) {
		database->setIntegerForKey("FIFTH_PLACE", Fourth);
		database->setIntegerForKey("FOURTH_PLACE", Third);
		database->setIntegerForKey("THIRD_PLACE", _scorevalue);
		database->flush();
	}
	else if (_scorevalue > Second&&_scorevalue <= First) {
		database->setIntegerForKey("FIFTH_PLACE", Fourth);
		database->setIntegerForKey("FOURTH_PLACE", Third);
		database->setIntegerForKey("THIRD_PLACE", Second);
		database->setIntegerForKey("SECOND_PLACE", _scorevalue);
		database->flush();
	}
		...
		database->setIntegerForKey("FOURTH_PLACE", Third);
		database->setIntegerForKey("THIRD_PLACE", Second);
		database->setIntegerForKey("SECOND_PLACE", First);
		database->setIntegerForKey("FIRST_PLACE", _scorevalue);
		database->flush();
	}
	...
	if (_scorevalue < target_score) {
		...
		auto nextBtn = MenuItemLabel::create(label4, CC_CALLBACK_1(level3::nextCallback, this));
...
void level3::nextCallback(Ref * pSender) {
